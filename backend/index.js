...
const addUser = async (profileId, socketId) => {
  ...
  if (!existingUser) {
    ...
    try {
      ...
      console.log(
        `Profile ${profileId} is set to online and lastLogin updated`
      );
    } catch (error) {
    ...
    console.log(
      "ProfileId already associated with a socket. No changes made.",
      users
    );
  }
...
const removeUser = async (socketId) => {
  ...
  if (user) {
    try {
      ...
      console.log(
        `Profile ${user.profileId} is set to offline and lastSeen updated`
      );
